document.addEventListener('DOMContentLoaded', () => {
    ...
    if (langBtn) {
        ...
        if (currentLang === 'ar') {
            ...
            setTimeout(() => triggerGoogleTranslate('ar'), 1500);
        } else {
...
(function initCommMega() {
    ...
    const feedLabel = userName ? `${userName.split(' ')[0]}'s Feed` : 'Your Feed';
    ...
    document.addEventListener('click', (e) => {
        if (!wrap.contains(e.target)) mega.classList.remove('rx-open');
    });
