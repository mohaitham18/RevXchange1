  ...
  try {
    ...
    if (!['buy', 'rent', 'appointment'].includes(type)) {
      return res.status(400).json({
        message: 'Type must be buy, rent, or appointment'
      });
    }

    if (!carId) {
      return res.status(400).json({
        message: 'Car reference ID is required.'
      });
    }
    ...
    if (!phone?.trim()) {
      return res.status(400).json({
        message: 'Phone contact baseline is required.'
      });
    }
    ...
    if (!/^[A-Za-z\s]+$/.test(name.trim())) {
      return res.status(400).json({
        message: 'The contact name field must contain only letters.'
      });
    }
    ...
    if (!/^(010|011|012|015)\d{8}$/.test(phoneDigits)) {
      return res.status(400).json({
        message: 'Enter a valid 11-digit Egyptian phone number starting with 010, 011, 012, or 015.'
      });
    }
    ...
    if (!car) {
      return res.status(404).json({
        message: 'The referenced vehicle could not be found.'
      });
    }
    ...
    if (type === 'rent') {
      if (!rentFrom || !rentTo) {
        return res.status(400).json({
          message: 'Rental window schedules are required.'
        });
      }

      if (new Date(rentFrom) >= new Date(rentTo)) {
        return res.status(400).json({
          message: 'The return date must conclude after the vehicle dropoff.'
        });
      }
    ...
    if (type === 'buy') {
      if (!appointmentDate) {
        return res.status(400).json({
          message: 'A specific checkout pickup date is required.'
        });
      }
    ...
    if (type === 'appointment') {
      if (!appointmentDate) {
        return res.status(400).json({
          message: 'An inspection appointment schedule date is required.'
        });
      }
    ...
    res.status(201).json({
      message: 'Your request was dispatched directly to the seller.',
      request
    });
  } catch (err) {
...
const getMyRequests = async (req, res) => {
  try {
    const requests = await Request.find({ user: req.user._id })
      .populate('car', 'brand model year price images rejectionReason status phone')
...
const getIncomingRequests = async (req, res) => {
  try {
    const requests = await Request.find({
      ...
      .populate('car', 'brand model year price images phone')
...
// ── Peer-to-Peer: Update Request Status ─────────────────────────
// Car owner accepts/rejects/resets a request
const ownerUpdateRequest = async (req, res) => {
  try {
    ...
    const request = await Request.findById(req.params.id)
      .populate('car', 'brand model year user phone')
      .populate('user', 'name email');
    ...
    if (!request.car) {
      return res.status(404).json({
        message: 'Car for this request was not found'
      });
    }
        ...
        await sendMail({
          ...
          html: `
            <div style="font-family:sans-serif;max-width:480px;margin:auto">
              ...
              <p>
                Your <strong>${request.type}</strong> request for
                <strong>${carTitle}</strong> was <strong>${ownerStatus}</strong>.
              </p>
              ...
              ${
                ownerStatus === 'accepted'
                  ? `<p>You can now contact the seller from your dashboard requests tab.</p>`
...
const adminUpdateRequest = async (req, res) => {
  try {
    ...
    if (!request) {
      return res.status(404).json({
        message: 'Request line row not found.'
      });
    }
    ...
    res.json({
      message: 'Global record updated tracking status successfully.',
      request
    });
  } catch (err) {
...
const adminDeleteRequest = async (req, res) => {
  try {
    ...
    if (!request) {
      return res.status(404).json({
        message: 'Request data target not found.'
      });
    }
    ...
    res.json({
      message: 'Request index permanent erasure complete.'
    });
  } catch (err) {
